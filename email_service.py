...
MÉTODO PRINCIPAL — API HTTP do Resend (recomendado, e obrigatório em
hospedagens como o Render no plano gratuito, que bloqueia conexões de
saída SMTP nas portas 587/465 com "OSError: Network is unreachable").
Como a API do Resend usa HTTPS (porta 443), ela passa sem problemas
pelo firewall dessas hospedagens.
...
Prioridade de envio: Resend (se RESEND_API_KEY definida) -> SMTP (se
configurado) -> modo simulado (log).
"""
import os
import json
import smtplib
...
if _resend_configurado():
    ...
    logger.warning(
        "E-mail: RESEND_API_KEY não encontrada — usando SMTP (%s:%s). "
...
def enviar_email(destinatario, assunto, corpo_texto):
    ...
    Ordem de tentativa: Resend (API HTTP) -> SMTP -> modo simulado.
    """
